...
/**
 * Delete one job out of the Windows queue.
 *
 * Remove-PrintJob throws if the job has already finished or been flushed, which
 * on the control page looked exactly like the button doing nothing. A job that
 * is no longer there is the outcome the staff member wanted, so report it as
 * done; anything else comes back as a sentence they can act on.
 */
export async function cancelJob(name, id) {
  const n = Number(id)
  if (!Number.isInteger(n)) throw new Error('เลขงานไม่ถูกต้อง')
  const out = await run(
    `$j = Get-PrintJob -PrinterName '${q(name)}' -ID ${n} -ErrorAction SilentlyContinue; ` +
    `if (-not $j) { 'gone'; exit }; ` +
    `try { Remove-PrintJob -PrinterName '${q(name)}' -ID ${n} -ErrorAction Stop; 'ok' } ` +
  ...
  // 'gone' - already off the queue by the time the button was pressed.
  return out === 'gone' ? 'gone' : 'ok'
}
