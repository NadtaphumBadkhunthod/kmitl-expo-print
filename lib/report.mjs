...
function block(sec, values, n) {
  const rows = sec.rows.map(r => row(r, values, sec.hue)).join('')
  if (!rows) return ''
  return `<section>
    <h2 style="background:${sec.tint}">
      <span class="n" style="background:${sec.hue}">${n}</span>
      <b style="color:${sec.hue}">${esc(sec.th)}</b><i>${esc(sec.en)}</i></h2>
...
export function buildHTML({ ticket, measuredAt, values,
  ...
  * { box-sizing: border-box; }
  ...
  h2 { display:flex; align-items:center; gap:7px; min-height:27px; margin:0; overflow:hidden;
       padding:0 10px; border-radius:6px 6px 0 0; white-space:nowrap; }
  /* The section number, as on the result page - two blocks in related colours
     are still told apart instantly by the badge. */
  h2 .n { flex:none; width:16px; height:16px; border-radius:50%; color:#fff;
          font-size:11.4px; font-weight:700; line-height:16px; text-align:center; }
  h2 b { font-size:14.7px; font-weight:700; flex:none; }
