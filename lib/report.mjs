...
function buildPage({ ticket, measuredAt, values, eventName, inkSaver = false, showTicket = true, lang = 'TH', mono = false }) {
  ...
  const legend = `<section class="legend">
    ...
    <div class="lg-l">${currentLegend.map(l =>
      `<span>${esc(l.label)}${l.sub ? `<i>${esc(l.sub)}</i>` : ''}</span>`).join('')}</div>
  </section>`
...
export function buildHTML(opts) {
  ...
  * { box-sizing: border-box; }
  ...
  .lg-bar i { flex:1 1 0; }
  .lg-l { display:flex; margin-top:1px; }
  .lg-l span { flex:1 1 0; text-align:center; font-size:11.8px; color:var(--leg);
               line-height:1.3; padding-top:2px; }
  /* The risk reading, only under the two bands that carry one. Thai needs the
     bleed - a tone mark on บรรทัดสอง sits above the Latin x-height and a tight
     line-height would clip it. */
  .lg-l i { display:block; font-style:normal; font-size:10.2px; line-height:1.25;
            color:var(--sub); padding:1px 0 0; white-space:nowrap; }
