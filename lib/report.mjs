...
export function buildHTML({ ticket, measuredAt, values, qrDataUrl, qrBig = false,
  ...
  const legend = `<section class="legend">
    <div class="lg-t">ความหมายของสี</div>
    ${LEGEND.map(ln => `<div class="lg-r">${ln.map(([k, label]) =>
      `<span><i style="background:${LEVELS[k].color}"></i>${esc(label)}</span>`).join('')}</div>`).join('')}
  </section>`

  const columns = COLUMNS.map((col, n) =>
    `<div class="col">${col.map(sec => block(sec, values)).join('')}${n === 1 ? legend : ''}</div>`
  ).join('')
  ...
  * { box-sizing: border-box; }
  ...
  /* ------------------------------------------------------------ header --- */
  header { display:flex; align-items:center; gap:10px; min-height:80px;
           flex:none;
  ...
  .brand .b1 { font-size:22.7px; font-weight:800; color:#f16627; line-height:1.1; }
  .brand .b1 span { color:#173a6a; font-size:18.7px; font-weight:700; margin-left:6px; }
  .brand .b2 { font-size:18.7px; font-weight:700; color:#173a6a; line-height:1.25; }
  .brand .b3 { font-size:14.7px; color:#5b6675; letter-spacing:.2px; }
  .qr { flex:none; text-align:center; }
  .qr img { width:65px; height:65px; display:block;
            image-rendering:pixelated; image-rendering:crisp-edges; }
  .qr.big img { width:88px; height:88px; }
  body.qrbig header { min-height:107px; }
  .qr p { margin:2px 0 0; font-size:12px; color:#6b7480; }

  /* --------------------------------------------- measurement conditions --- */
  .meta { display:flex; gap:8px; min-height:38px; margin-top:6px; }
  .chip { flex:1 1 0; display:flex; align-items:center; gap:9px; white-space:nowrap;
          padding:0 14px; background:#f5f7fa; border:1px solid #e6ebf2; border-radius:7px; }
  .chip span { font-size:14.7px; color:#9ba3b0; }
  .chip b { font-size:14.7px; font-weight:700; color:#173a6a; }
  .chip.tk { flex:0 0 auto; }
  .chip.tk b { font-size:18.7px; }
  .stars { letter-spacing:2px; color:#159a52; font-size:15px; }
  ...
  .lg-r { display:flex; gap:16px; margin-top:3px; line-height:1.3; }
  .lg-r span { display:flex; align-items:center; gap:6px; font-size:12.8px; color:#3e4349; }
  .lg-r i { width:11px; height:11px; border-radius:2px; flex:none; }
  ...
  <header>
    ...
    <div class="brand">
      ...
      <div class="b3">Passive &middot; Infrared &middot; Precision &middot; Examination &middot; Kit</div>
    </div>
